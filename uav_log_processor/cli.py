...
def validate_files(files: List[str]) -> List[str]:
    ...
    for file_path in files:
        ...
        if not path.suffix.lower() in ['.tlog', '.bin', '.rlog', '.txt', '.log']:
            print(f"Warning: Unsupported file format: {file_path}", file=sys.stderr)
            continue
...
def main():
    ...
    try:
        ...
        if args.input_dir:
            ...
            if not files:
                print(f"No log files found in directory: {args.input_dir}", file=sys.stderr)
                return 1
        else:
            files = args.files
        ...
        if args.validate_only:
            print(f"Validation complete. Found {len(valid_files)} valid log files.")
            return 0
        ...
        if not file_handler.validate_output_directory(config.output_dir):
            print(f"Error: Cannot write to output directory: {config.output_dir}", file=sys.stderr)
            return 1
        
        # Initialize processor and run
        logger.info(f"Starting UAV log processing with {len(valid_files)} files")
