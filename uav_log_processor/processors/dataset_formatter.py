...
class DatasetFormatter(BaseFormatter):
    ...
    def standardize_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if column_mapping:
            ...
            logger.info(f"Mapped {len(column_mapping)} columns to standard names")
        ...
        available_columns = [col for col in self.STANDARD_COLUMNS if col in standardized_data.columns]
        standardized_data = standardized_data[available_columns]
        
        logger.info(f"Standardized dataset with {len(available_columns)} features")
    ...
    def _create_column_mapping(self, columns: List[str]) -> Dict[str, str]:
        ...
        imu_mappings = {
            'accel_x': 'imu_ax', 'acc_x': 'imu_ax', 'ax': 'imu_ax',
            'accel_y': 'imu_ay', 'acc_y': 'imu_ay', 'ay': 'imu_ay',
            'accel_z': 'imu_az', 'acc_z': 'imu_az', 'az': 'imu_az',
        ...
        quality_mappings = {
            'horizontal_dop': 'hdop', 'h_dop': 'hdop',
            'vertical_dop': 'vdop', 'v_dop': 'vdop',
            'gps_fix_type': 'fix_type', 'fix': 'fix_type'
        }
        ...
        for feature in self.CATEGORICAL_FEATURES:
            if feature in data.columns:
                if feature == 'fix_type':
                    ...
                    encoded_data[feature] = data[feature].astype(int)
