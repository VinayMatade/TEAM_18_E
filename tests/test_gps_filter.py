...
class TestGPSReliabilityFilter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.filter = GPSReliabilityFilter(
            min_fix_type=3, max_hdop=5.0, max_vdop=10.0)
    ...
    def test_calculate_gps_quality_score_high_quality(self):
        ...
        df = pd.DataFrame({
            ...
            # Stable position
            'gps_lat': 37.123456 + np.random.normal(0, 0.00001, n_samples),
    ...
    def test_calculate_gps_quality_score_low_quality(self):
        ...
        df = pd.DataFrame({
            ...
            # High HDOP
            'hdop': [8.0, 10.0, 9.0, 12.0, 7.0] * (n_samples // 5),
            # High VDOP
            'vdop': [15.0, 20.0, 18.0, 25.0, 12.0] * (n_samples // 5),
            # Unstable position
            'gps_lat': 37.123 + np.random.normal(0, 0.001, n_samples),
    ...
    def test_get_gps_quality_report_comprehensive(self):
        ...
        # 4 pass all filters (fix_type>=3, hdop<=5, vdop<=10)
        self.assertEqual(report['reliable_samples'], 4)
        # 4 out of 6 samples
        self.assertAlmostEqual(report['reliability_ratio'], 4/6)
...
class TestGPSUtilityFunctions(unittest.TestCase):
    ...
    def test_detect_gps_outages_with_gaps(self):
        ...
        timestamps = [0.0, 0.1, 0.2, 5.0, 5.1, 10.0,
                      10.1]  # Gaps at 0.2->5.0 and 5.1->10.0
        df = pd.DataFrame({
