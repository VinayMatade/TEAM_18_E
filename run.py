...
OUTPUT_CSV = os.getenv("OUTPUT_CSV", "corrected_flight.csv")
SKIP_PLOT = os.getenv("SKIP_PLOT", "0") == "1"
...
def run_inference():
    ...
    model = TCN(input_size=9, output_size=2, num_channels=[
                64, 128, 64], kernel_size=3, dropout=0.0)
    ...
    with torch.no_grad():
        ...
        for i in range(len(full_input) - SEQ_LEN):
            ...
            x_tensor = torch.tensor(
                window, dtype=torch.float32).unsqueeze(0).to(DEVICE)
    ...
    if not SKIP_PLOT:
        ...
        plt.plot(df['GPS_Lng'], df['GPS_Lat'],
                 label='Original (Noisy)', alpha=0.5, color='red')
        plt.plot(df['Corrected_Lng'], df['Corrected_Lat'],
                 label='AI Corrected', linewidth=2, color='green')
